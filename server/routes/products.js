...
router.post('/', auth, async (req, res) => {
    try {
        const { category_id, name, short_description, description, base_price, mrp, min_order_qty, unit_type, tags, is_featured, image_url, gallery_images, pricing_tiers, custom_options, offer_id, gst_percent, stock, no_return } = req.body;
        const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        
        const [result] = await db.execute(
            'INSERT INTO products (category_id, name, slug, short_description, description, base_price, mrp, min_order_qty, unit_type, image_url, gallery_images, tags, is_featured, custom_options, offer_id, gst_percent, stock, no_return) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [category_id, name, slug, short_description, description, base_price, mrp || null, min_order_qty || 1, unit_type || 'pcs', image_url || null, gallery_images ? JSON.stringify(gallery_images) : null, tags, is_featured ? 1 : 0, custom_options || null, offer_id || null, gst_percent || 18.00, stock !== undefined && stock !== '' && stock !== null ? parseInt(stock) : null, no_return ? true : false]
        );
...
router.put('/:id', auth, async (req, res) => {
    try {
        const { category_id, name, short_description, description, base_price, mrp, min_order_qty, unit_type, tags, is_featured, is_active, image_url, gallery_images, pricing_tiers, custom_options, offer_id, gst_percent, stock, no_return } = req.body;
        const updates = { 
            category_id, name, short_description, description, base_price, mrp: mrp || null, min_order_qty, 
            unit_type: unit_type || 'pcs', tags, is_featured: is_featured ? 1 : 0, is_active: is_active ? 1 : 0, 
            offer_id: offer_id || null, gst_percent: gst_percent || 18.00,
            stock: stock !== undefined && stock !== '' && stock !== null ? parseInt(stock) : null,
            no_return: no_return ? true : false
        };
